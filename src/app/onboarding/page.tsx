...
const LEAGUE_LABELS: Record<string, string> = { nfl: "NFL", nba: "NBA", mlb: "MLB", nhl: "NHL" };

function TeamTile({ team, selected, onToggle }: { team: Team; selected: boolean; onToggle: (id: string) => void }) {
  ...
  return (
    <button
      ...
      style={
        selected
          ? { background: `linear-gradient(135deg,${color}28,${color}0e)`, border: `1.5px solid ${color}`, boxShadow: `0 0 0 1px ${color}60` }
          : { background: "var(--surface-200)", border: "1px solid var(--surface-300)" }
      }
    >
      <span className="text-2xl leading-none">{team.logo}</span>
      <span className="text-[9px] font-bold leading-tight line-clamp-1 max-w-full" style={{ color: selected ? color : "var(--surface-muted)" }}>
        {team.name.split(" ").at(-1)}
      </span>
      {selected && (
        <span className="absolute top-1 right-1 w-3 h-3 rounded-full flex items-center justify-center" style={{ background: color }}>
...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    ...
    const tid = setTimeout(async () => {
      ...
      else                     { setUsernameStatus("taken");     setUsernameError("That username is already taken."); }
    }, 450);
  ...
  if (!isLoaded) {
    return <div className="flex items-center justify-center p-12"><div className="w-8 h-8 border-2 border-brand border-t-transparent rounded-full animate-spin" /></div>;
  }

  const firstName = user?.firstName ?? user?.fullName?.split(" ")[0] ?? "there";
  const leagueGroups = ["nfl","nba","mlb","nhl"].map((lid) => ({
    id: lid, label: LEAGUE_LABELS[lid], teams: ONBOARDING_TEAMS.filter((t) => t.leagueId === lid),
    ...
    <div className="w-full max-w-lg">
      ...
      <div className="text-center mb-8">
        <div className="flex items-center justify-center gap-2 mb-2">
          ...
          <span className="text-xl font-black tracking-tighter text-white">UNDRAFTED</span>
        </div>
        <p className="text-sm text-white/50">
          {step === 1 ? `Hey ${firstName} — pick a username` : "Pick your teams"}
        </p>
      </div>
      ...
      <div className="flex items-center justify-center gap-2 mb-8">
        {[1, 2].map((s) => (
          <div key={s} className="h-1 w-12 rounded-full transition-colors"
            style={{ background: s <= step ? "var(--brand)" : "rgba(255,255,255,0.15)" }} />
        ))}
      </div>
      ...
      {step === 1 && (
        <form onSubmit={handleStep1} className="bg-surface-200 border border-surface-300 rounded-2xl p-6 space-y-5">
          <h2 className="text-base font-black text-surface-text">Choose your username</h2>
          <p className="text-xs text-surface-muted -mt-2">Your display name comes from your Clerk account — change it in the profile menu anytime.</p>
          ...
          {step1Error && (
            <p className="text-xs text-red-500 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">{step1Error}</p>
          )}

          <button type="submit" className="w-full py-2.5 bg-brand hover:bg-brand/90 text-white font-bold rounded-xl text-sm transition-colors">
            Continue →
          </button>
        </form>
      )}
      ...
      {step === 2 && (
        <form onSubmit={handleSubmit} className="bg-surface-200 border border-surface-300 rounded-2xl overflow-hidden">
          <div className="px-6 pt-6 pb-4">
            <h2 className="text-base font-black text-surface-text mb-1">Pick your teams</h2>
            <p className="text-xs text-surface-muted">{selectedTeams.size} selected · choose at least 1</p>
          </div>

          <div className="px-6 pb-4 space-y-6 max-h-[55vh] overflow-y-auto">
            {leagueGroups.map(({ id, label, teams: leagueTeams }) => (
              <section key={id}>
                <p className="text-[10px] font-black uppercase tracking-widest text-surface-muted mb-2">{label}</p>
                <div className="grid grid-cols-5 sm:grid-cols-6 gap-2">
                  {leagueTeams.map((team) => (
                    <TeamTile key={team.id} team={team} selected={selectedTeams.has(team.id)} onToggle={toggleTeam} />
                  ))}
                </div>
              </section>
            ))}
          ...
          {submitError && (
            <div className="mx-6 mb-3">
              <p className="text-xs text-red-500 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">{submitError}</p>
            </div>
          )}

          <div className="px-6 py-4 border-t border-surface-300 flex items-center gap-3">
            <button type="button" onClick={() => setStep(1)} className="text-xs text-surface-muted hover:text-surface-text transition-colors">← Back</button>
            <button type="submit" disabled={submitting || selectedTeams.size === 0}
