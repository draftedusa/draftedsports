...
export default function LeagueGlossary({ type }: { type: string }) {
  ...
  return (
    <div className="mt-12 rounded-lg border border-white/10 bg-zinc-900/50 p-6">
      <div className="mb-4 flex items-center gap-2">
        <Info size={16} className="text-red-500" />
        <h3 className="text-xs font-black uppercase tracking-widest text-white">Glossary &amp; Key</h3>
      </div>
      <div className="grid grid-cols-1 gap-x-8 gap-y-4 md:grid-cols-2 lg:grid-cols-4">
        {items.map((item) => (
          <div key={item.term}>
            <span className="text-[10px] font-black text-white">{item.term}</span>
            <p className="text-[10px] leading-relaxed text-white/40">{item.definition}</p>
          </div>
        ))}
      </div>
    </div>
