...
const SOCIAL_LINKS = [
  {
    ...
    label: "YouTube",
    color: "hover:text-red-500 hover:border-red-500/40 hover:bg-red-500/5",
    icon: (
  ...
  },
  {
    ...
    label: "Instagram",
    color: "hover:text-pink-500 hover:border-pink-500/40 hover:bg-pink-500/5",
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
  ...
  },
  {
    href: "https://x.com",
    label: "X / Twitter",
    color: "hover:text-surface-text hover:border-surface-300 hover:bg-surface-300",
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
...
export default function LeftRail() {
  ...
  return (
    <aside className="hidden lg:flex flex-col w-56 shrink-0 py-6 sticky top-[120px] self-start h-[calc(100vh-120px)] overflow-y-auto no-scrollbar">

      {/* QUICK LINKS */}
      <div className="mb-6">
        <p className="text-[10px] font-bold uppercase tracking-widest text-surface-muted px-3 mb-2">
          Quick Links
        </p>
        <div className="space-y-0.5">
          {QUICK_LINKS.map(({ href, label, icon }) => {
            const active = pathname === href && label !== "NBA G League" && label !== "NFL 1st Round Mock Draft";
            return (
              <Link
                key={label}
                href={href}
